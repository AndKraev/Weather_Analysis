...
def test_weatheranalysis_get_city_centers(weatheranalysis):
    test_data = {("FR", "Paris"): pd.DataFrame({"Latitude": [2, 4], "Longitude": [3, 5]})}
...
def test_weatheranalysis_create_charts(weatheranalysis):
    Path(weatheranalysis.output_path / "FR" / "Paris").mkdir(parents=True, exist_ok=True)
    df_1 = pd.DataFrame(
        {
            "Latitude": (2, 4),
            "Longitude": (3, 5),
    ...
    with patch("WeatherAnalysis.PickPoint") as mocked:
        ...
        result = pd.read_csv(weatheranalysis.output_path / "FR" / "Paris" / "Hotels.csv")
...
def test_weatheranalysis_run(
        FileHandler, find_cities_with_most_hotels, get_most_dfs, get_city_centers,
        OpenWeather, create_charts, create_json_wth_analysis, hotels_to_csv, weatheranalysis
):
    weatheranalysis.run()
